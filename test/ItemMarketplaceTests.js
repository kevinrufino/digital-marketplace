...
// to acquire Phunks for testing, a cloned TownsNFT contract is deployed to mint some
describe("ItemMarketplace Contract", function() {
  let phunks, phunksMarket, openMining, deployer, addr1, addr2
  before(async function() {
    [deployer, addr1, addr2] = await ethers.getSigners();
    // console.log("Deployer: ", deployer.address)
    // console.log("Addr1: ", addr1.address)
    phunks = await helpers.deployContract(false, "TownsNFT")
    phunksMarket = await helpers.deployContract(false, "ItemMarketplace", phunks.address)
    openMining = await helpers.deployContract(false, "OpenMining")

    await openMining.setAddresses(phunks.address);

    // Minting 10 Towns for testing
    await phunks.startSale()
    await phunks.connect(addr1).mint(10, { value: utils.parseUnits("0.0", "ether") })
  })
  ...
  describe("Test offerPhunkForSale", function() {
    ...
    it("All phunks for sale", async function() {
      let offer = await phunksMarket.getAllForSale()
      console.log("------------phunks offered for sale---------", offer)
      expect(offer.length).to.equal(5)
    })
  })
