...
contract ItemMarketplace is ReentrancyGuard, Pausable, Ownable {
    ...
    //TODO check if number can be negative
    uint phunksForSaleCount;
    ...
    function getAllForSale() public view returns (Offer[] memory){
        Offer[] memory ret = new Offer[](phunksForSaleCount);
        for (uint i = 0; i < phunksForSaleCount; i++) {
            ret[i] = phunksOfferedForSale[i];
        }
        return ret;
    }

    function getAllBids() public view returns (Bid[] memory){
        Bid[] memory ret = new Bid[](phunkBidsCount);
        for (uint i = 0; i < phunkBidsCount; i++) {
            ret[i] = phunkBids[i];
        }
        return ret;
    }
